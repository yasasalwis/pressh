...
| R-11 | Plugin `presets.json` not covered by signature (only `main` is) | Low        | Med    | Signature now covers manifest-referenced aux files via a `files` map — host rejects a tampered/missing presets file at load; plus load-time sanitization + the no-inline renderer | Security | Mitigated |
| R-12 | Customer order PII (name/email/address) widens GDPR scope       | Med        | Med    | Orders carry a top-level `subjectRef` (lowercased email); `inventory_orders` registered in the GDPR subject scopes (export + erase) with `createdAt` for retention                | Eng      | Mitigated |
