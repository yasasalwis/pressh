/**
 * Curated inline-SVG icon set. All paths are author-controlled constants (no
 * user input reaches the SVG markup), and every icon draws with `currentColor`
 * so the `color` style controls it. Unknown names render nothing.
 */
...
/** Returns safe inline SVG for `name`, or "" when the name is unknown. */
export function renderIcon(name: string): string {
  const paths = ICON_PATHS[name];
  if (!paths) return "";
  return `<svg viewBox="${VB}" ${COMMON} width="100%" height="100%" aria-hidden="true" focusable="false">${paths}</svg>`;
}
